...
public class Status implements Serializable {
    ...
    /** 微博配图缩略图地址。多图时返回多图链接。无配图返回"[]" */
    public ArrayList<String> pic_urls;
    /** 微博配图原图地址。多图时返回多图链接。无配图返回"[]" */
    public ArrayList<String> original_urls;
    /** 微博配图中图地址。多图时返回多图链接。无配图返回"[]" */
    public ArrayList<String> bmiddle_urls;
    /** 微博流内的推广微博ID */
    //public Ad ad;
    ...
    public static Status parse(JSONObject jsonObject) {
        ...
        if (picUrlsArray != null && picUrlsArray.length() > 0) {
            ...
            status.original_urls = new ArrayList<String>(length);
            status.bmiddle_urls = new ArrayList<String>(length);
            JSONObject tmpObject = null;
            for (int ix = 0; ix < length; ix++) {
                tmpObject = picUrlsArray.optJSONObject(ix);
                if (tmpObject != null) {
                    String picurl = tmpObject.optString("thumbnail_pic");
                    status.pic_urls.add(picurl);
                    status.original_urls.add(picurl.replace("thumbnail", "large"));
                    status.bmiddle_urls.add(picurl.replace("thumbnail", "bmiddle"));
                }
            }
        }
